...
const snippetsPath = "snippets.json";
const snippet = JSON.parse(fs.readFileSync(snippetsPath, "utf8"));
const match = /^(\d+)\.(\d+)\.(\d+)$/.exec(snippet.version);

if (!match) {
  throw new Error(`Invalid manifest version: ${snippet.version}`);
}
...
fs.writeFileSync(snippetsPath, `${JSON.stringify(snippet, null, 2)}\n`);
...
const snippets = JSON.parse(fs.readFileSync("snippets.json", "utf8"));
